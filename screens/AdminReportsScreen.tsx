...
export default function AdminReportsScreen({navigation}: any) {
  const [tab, setTab] = useState<
    'reports' | 'users' | 'verifications' | 'casting'
  >('reports');
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      if (tab === 'reports') {
        ...
        const res = await api.get<{requests: any[]}>(
  ...
  const approveVerification = async (id: string) => {
    try {
      await api.put(`/admin/verification-requests/${id}`, {status: 'approved'});
      fetchData();
    } catch (e: any) {
      Alert.alert('Error', e.message);
    }
  ...
  const rejectVerification = async (id: string) => {
    try {
      await api.put(`/admin/verification-requests/${id}`, {status: 'rejected'});
      fetchData();
    } catch (e: any) {
      Alert.alert('Error', e.message);
    }
  ...
  const updateCastingRequest = async (
    id: string,
    newStatus: string,
    userEmail: string,
  ) => {
    try {
      ...
      Alert.alert(
        ...
        `${userEmail} has been ${
          newStatus === 'approved'
            ? 'approved as a Casting Director'
            : 'rejected'
        }.`,
      );
      fetchData();
      ...
      {authorized === false ? (
        ...
        <>
          <ScrollView
            ...
            {(['reports', 'users', 'verifications', 'casting'] as const).map(
              t => (
                <TouchableOpacity
                  key={t}
          ...
          <ScrollView contentContainerStyle={styles.scroll}>
              ...
              (reports.length === 0 ? (
                ...
                reports.map((r: any) => (
                  <Card
                    ...
                    {r.message ? (
                      <Text style={styles.detail}>{r.message}</Text>
                    ) : null}
                    {r.status === 'pending' && (
                      <View style={styles.actions}>
                        <Button
                          ...
                          style={styles.flex1}
                          onPress={() =>
                            updateReport(r._id || r.id, 'action_taken')
                          }
                        />
                        <Button
                          label="❌ Dismiss"
                          ...
                          style={styles.flex1}
                          onPress={() =>
                            updateReport(r._id || r.id, 'dismissed')
                          }
                        />
                      </View>
                    )}
              ...
              users.map((u: any) => (
                <Card
                  ...
                  <View style={styles.userRow}>
                    <Avatar name={u.fullName || u.email || 'User'} size="sm" />
                  ...
                  <View style={styles.actions}>
                    {u.banned ? (
                      <Button
                        ...
                        size="sm"
                        style={styles.flex1}
                        onPress={() => toggleBan(u._id || u.id).then(fetchData)}
                      />
                    ) : (
                      <Button
                        label="🚫 Ban"
                    ...
                    {!u.isApprovedDirector && (
                      <Button
                        ...
                        onPress={async () => {
                          try {
                            await api.put(`/admin/users/${u._id || u.id}`, {
                            ...
                            Alert.alert(
                              '✅ Approved',
                              `${u.email} is now a Casting Director.`,
                            );
                          } catch (e: any) {
                            Alert.alert('Error', e.message);
                          }
                  ...
                  {v.status === 'pending' && (
                    <View style={styles.actions}>
                      <Button
                        ...
                        size="sm"
                        style={styles.flex1}
                        onPress={() => approveVerification(v._id || v.id)}
                      />
                      <Button
                        label="❌ Reject"
                        variant="danger"
                        size="sm"
                        style={styles.flex1}
                        onPress={() => rejectVerification(v._id || v.id)}
                      />
                    </View>
                  )}
                </Card>
            ...
            {tab === 'casting' &&
              (castingRequests.length === 0 ? (
                <EmptyState
                  icon="🎬"
                  title="No casting director requests"
                  subtitle="When directors apply, they'll appear here"
                />
              ) : (
                castingRequests.map((cr: any) => (
                  <Card
                    key={cr._id || cr.id}
                    variant="default"
                    padding={Spacing.md}
                    ...
                    <View style={styles.cardHeader}>
                      <Text style={styles.userName}>
                        {cr.userName || cr.userEmail}
                      </Text>
                      <Badge
                        label={cr.status || 'pending'}
                        variant={
                          cr.status === 'approved'
                            ? 'success'
                            : cr.status === 'rejected'
                    ...
                    {cr.companyName ? (
                      <Text style={styles.meta}>🏢 {cr.companyName}</Text>
                    ) : null}
                    {cr.yearsExperience ? (
                      <Text style={styles.meta}>
                        📅 {cr.yearsExperience} years experience
                      </Text>
                    ) : null}
                    {cr.message ? (
                      <Text style={styles.detail}>{cr.message}</Text>
                    ) : null}
                    {cr.phone ? (
                      <Text style={styles.meta}>📱 {cr.phone}</Text>
                    ) : null}
                    {cr.idProofUrl ? (
                      <TouchableOpacity
                        onPress={() =>
                          navigation.navigate('ImageViewer', {
                            imageUrl: cr.idProofUrl,
                          })
                        }
                    ...
                    {cr.portfolio ? (
                      <Text style={styles.meta}>🔗 {cr.portfolio}</Text>
                    ) : null}
                    <Text style={styles.meta}>
                      Applied: {formatDate(cr.createdAt)}
                    </Text>
                    {cr.status === 'pending' && (
                      <View style={styles.actions}>
                        <Button
