...
export default function SuggestedFollowsScreen({navigation}: any) {
  ...
  const toggleFollow = async (userId: string) => {
    ...
    catch { setFollowing(prev => { const n = new Set(prev); isFollowing ? n.add(userId) : n.delete(userId); return n; }); }
  };

  return (
    <View style={styles.safe}>
      <Header title="👥 Follow Suggestions" />
          ...
          renderItem={({item}) => {
            ...
            return (
              <View style={styles.userRow}>
                ...
                <Button label={following.has(uid) ? 'Following' : '+ Follow'} variant={following.has(uid) ? 'secondary' : 'primary'} size="sm" onPress={() => toggleFollow(uid)} />
              </View>
            );
          }}
        />
