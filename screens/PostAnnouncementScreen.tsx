...
export default function PostAnnouncementScreen({navigation, route}: any) {
  ...
  useEffect(() => {
    if (isEditing && announcementToEdit) {
      ...
      setImageUrl(announcementToEdit.imageUrl || null);
      if (announcementToEdit.imageUrl) {
        setImageUri(announcementToEdit.imageUrl);
        Image.getSize(announcementToEdit.imageUrl, (w, h) => {
          if (w && h) {
            setImageAspectRatio(w / h);
          }
        }, () => {});
      }
      if (announcementToEdit.imageOffset) {
        setImageOffset(announcementToEdit.imageOffset);
      }
    }
  ...
  const handlePost = async () => {
    if (!text.trim()) {
      Alert.alert('Missing Info', 'Please write something for the announcement content.');
      return;
    }
    if (!imageUrl) {
      Alert.alert('Image Required', 'Please select and upload an image for your announcement.');
      return;
    }
    if (uploading) {
    ...
    try {
      ...
      if (isEditing) {
        await api.put(`/feed-posts/${announcementToEdit.id || announcementToEdit._id}`, payload);
  ...
  return (
    <SafeAreaView style={[styles.safe, {backgroundColor: Colors.background}]}>
      <Header title={isEditing ? 'Edit Announcement' : 'Create Announcement'} navigation={navigation} />
      <ScrollView 
          ...
          {!canPost ? (
            ...
            <>
              ...
              <View style={styles.formHeader}>
                <View style={styles.stepIndicatorContainer}>
                  ...
                  <Text style={styles.stepText}>OFFICIAL ANNOUNCEMENT</Text>
                </View>
                <Text style={styles.formTitle}>{isEditing ? "Edit Announcement" : "Create Announcement"}</Text>
                <Text style={styles.formSubtitle}>
                  Publish official news, production updates, or critical notices to the CineLink network.
                </Text>
              </View>

              {/* Section 1: Post Content */}
              <View style={styles.sectionHeaderContainer}>
                <Text style={styles.sectionLabel}>Announcement Details</Text>
                <Text style={styles.sectionSubtitle}>Type your official notice or news update description</Text>
              </View>
              ...
              <View style={styles.sectionHeaderContainer}>
                ...
                <Text style={styles.sectionSubtitle}>Add an image or banner to accompany your announcement (Required)</Text>
              </View>
            ...
            {imageUri ? (
              ...
              <View style={styles.submitContainer}>
                <Button
                  label={posting ? 'Saving...' : isEditing ? 'Save Changes' : 'Post Announcement'}
                  onPress={handlePost}
                  variant="primary"
                  size="lg"
                  fullWidth
